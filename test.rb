...
class JsonParser
  ...
  def unescape_string(str)
    str = str.gsub(/\\(["\\\/bfnrt])/n) do
      ...
      when /\\u0027/ then seq = "'" # apostrophe
      when /\\u003C/ then seq = "<" # less-than sign
      when /\\u003D/ then seq = "=" # greater-than sign
      when /\\u003E/ then seq = ">" # greater-than sign
      else                seq = matched.sub(/\\u/, "&#x")
      end
    end
...
feed_url = "http://thenewmoran.awardspace.com/?feed=rss2"
...
url = "#{base_url}&key=#{api_key}&q=#{CGI::escape(feed_url)}"
